// 添加 --host 参数可验证实际大屏添加/挂载函数、GridStack 和 WidgetHost。
...
createApp({setup(){
let grid;const widgets=ref({}),layout=ref([]),editorMode=ref('editing'),isSavingEdit=ref(false),nativePickerVisible=ref(false),nativeEditSource=ref(null),addPanelVisible=ref(false),mountedApps=new Map();
...
return()=>h('main',{'data-v-map-fixture':''},[h('button',{onClick:seed},'添加测试温度卡片'),h('button',{onClick:()=>{editorMode.value=editorMode.value==='editing'?'view':'editing';if(editorMode.value==='view'){const saved=normalizeMapTemplateState(JSON.parse(JSON.stringify({layout:layout.value,widgets:widgets.value,viewport:templateViewport.value})));templateViewport.value=saved.viewport;layout.value=saved.layout;renderGrid()};grid.setStatic(editorMode.value==='view');grid.enableResize(editorMode.value==='editing')}},'切换编辑/查看'),h('p',JSON.stringify({layout:layout.value,viewport:templateViewport.value})),h('h2','实际 GridStack + WidgetHost 验收 · 测试数据'),h('button',{onClick:()=>nativePickerVisible.value=true},'打开原生部件库'),h('p',errorMsg.value),h('p','已添加 '+Object.keys(widgets.value).length+' 个部件'),h('div',{style:'position:relative;height:420px;overflow:hidden',class:'fixture-stage'},[h('div',{class:['mw-grid grid-stack',{'mw-grid--editing':editorMode.value==='editing'}],style:'width:100%;height:420px','data-v-map-fixture':'',ref:el=>{if(!el||grid)return;GridStack.renderCB=(el,w)=>el.innerHTML=w.content||'';grid=GridStack.init({column:24,cellHeight:150,float:true,alwaysShowResizeHandle:true,resizable:{handles:'all'}},el);grid.on('resizestart',beginWidgetResize);grid.on('change',()=>{syncLayoutFromGrid();applyScreenMetrics()})}})]),h(Picker,{visible:nativePickerVisible.value,onClose:()=>nativePickerVisible.value=false,onConfirm:applyNativeWidget})]);}}).mount('#app');`
  ...
  plugins: [
    {
      ...
      setup(builder) {
        builder.onResolve(
          { filter: /^\/@\/api\/tb\/(device|asset|deviceProfile|telemetry)$|^\/@\/store\/modules\/user$/ },
...
createServer((request, response) => {
  ...
  if (!['/app.js', '/app.css'].includes(request.url)) {
