...
export interface ServerSideRpcRequest {
  method: string;
  params?: Recordable | string | number | boolean | null;
  timeout?: number;
}
...
export function rpcSendServerSideTwoway(deviceId: string, data: ServerSideRpcRequest) {
  return defHttp.postJson<RpcSendResult>(
    {
...
export function rpcSendServerSideOneway(deviceId: string, data: ServerSideRpcRequest) {
  return defHttp.postJson<RpcSendResult>(
    {
