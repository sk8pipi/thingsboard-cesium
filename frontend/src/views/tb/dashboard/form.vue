...
<script lang="ts" setup name="ViewsTbDashboardForm">
  ...
  import { DASHBOARD_MAP_WIDGET_CONFIG_KEY, createDefaultMapTemplateState } from '/@/views/tb/map/mapTemplateConfig';
  ...
  async function handleSubmit() {
    try {
      ...
      if (!record.value.id?.id && !configuration[DASHBOARD_MAP_WIDGET_CONFIG_KEY]) {
        configuration[DASHBOARD_MAP_WIDGET_CONFIG_KEY] = createDefaultMapTemplateState();
      }

      const res = await saveDashboard({ ...data, id: record.value.id, configuration });
