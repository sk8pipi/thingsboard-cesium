<template>
  <div class="tb-card">
    ...
    <div v-else-if="!canRender" class="tb-empty">
      Polar area 至少需要 1 个 key
    </div>
    ...
    <div v-if="displayItems.length" class="tb-footer">
      <div class="tb-legend">
        ...
        <div class="tb-legend-item tb-legend-item-total">
...
const polarSettings = computed(() => {
  return (
    props.config?.tbPolarArea ||
    props.widget?.config?.tbPolarArea ||
    props.config?.settings?.tbPolarArea ||
    props.widget?.config?.settings?.tbPolarArea ||
    {}
  );
});
...
const displayItems = computed<DisplayItem[]>(() => {
  let items = dataKeyDefs.value.map((item) => ({
    name: item.name,
    label: item.label,
    color: item.color,
  ...
  const sort = polarSettings.value.sort || 'desc';
...
const canRender = computed(() => displayItems.value.length > 0);
...
function buildOption(): echarts.EChartsOption {
  const inner = Number(polarSettings.value.radiusInner ?? 35);
  const outer = Number(polarSettings.value.radiusOuter ?? 110);
  const showLegend = polarSettings.value.showLegend !== false;
  const showToolbox = polarSettings.value.showToolbox === true;
  const showLabel = polarSettings.value.showLabel !== false;
  ...
  return {
    ...
    tooltip: {
      ...
      formatter(params: any) {
        ...
        return `${data.name ?? ''}: ${formatNumber(data.value ?? 0)}`;
      },
    },
    series: [
      {
        ...
        radius: [inner, outer],
        center: ['50%', '45%'],
        roseType: polarSettings.value.roseType || 'area',
        itemStyle: {
          borderRadius: Number(polarSettings.value.borderRadius ?? 8),
        },
        label: {
          show: showLabel,
          color: '#fff',
        ...
        data: displayItems.value.map((item) => ({
          value: item.value,
          name: item.label,
          itemStyle: {
            color: item.color,
...
async function renderChart() {
  ...
  try {
    ...
    console.error('[TbLatestPolarArea] render failed:', err);
    errorMessage.value = err?.message || 'Polar area 渲染失败';
    disposeChart();
  }
}
