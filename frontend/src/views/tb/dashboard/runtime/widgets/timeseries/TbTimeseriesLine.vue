...
<script setup lang="ts">
  ...
  const selectedWindowMs = ref<number>(300000); // 默认 5min
  ...
  function buildSeries() {
    ...
    return ks.map((k) => {
      const pts = (all[k] || []).map((p) => [p.ts, p.value] as [number, number | string | null]);
      return { name: k, type: 'line', showSymbol: false, data: pts };
    });
  }
