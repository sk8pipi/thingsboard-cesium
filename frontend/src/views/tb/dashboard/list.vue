<template>
  <div class="dashboard-list">
    <BasicTable @register="registerTable">
      ...
      <template #selectedMapTemplate="{ record }">
        <Tag v-if="isSelectedMapTemplate(record)" color="processing">当前大屏</Tag>
        <span v-else>-</span>
      </template>
    </BasicTable>
...
<script lang="ts" setup>
  ...
  import { loadSelectedMapTemplateId, saveSelectedMapTemplateId } from '/@/views/tb/map/selectedMapTemplateStorage';
  ...
  const currentUserId = userStore.getUserInfo?.id?.id || '';
  const selectedMapTemplateId = ref(loadSelectedMapTemplateId(currentUserId));
  const tableColumns: BasicColumn[] = [
    ...
    {
      title: '当前大屏',
      dataIndex: 'selectedMapTemplate',
      key: 'selectedMapTemplate',
      width: 100,
      align: 'center',
      slot: 'selectedMapTemplate',
      ifShow: hasPermission(Authority.CUSTOMER_USER),
    },
  ];
    ...
    actions: (record: Recordable) => [
      {
        ...
        color: isSelectedMapTemplate(record) ? 'success' : undefined,
        title: isSelectedMapTemplate(record) ? '当前大屏' : '设为大屏',
        ifShow: hasPermission(Authority.CUSTOMER_USER),
        disabled: isSelectedMapTemplate(record),
        onClick: handleSetMapHomeTemplate.bind(this, { ...record }),
      },
      {
  ...
  async function handleDelete(record: Recordable) {
    createConfirm({
      iconType: 'error',
      title: t('tb.dashboard.action.deleteConfirm', { name: record.title }),
  ...
  function handleAssignToPublic(record: Recordable) {
    createConfirm({
      iconType: 'info',
      title: t('tb.dashboard.action.setPublicConfirm', { name: record.title }),
  ...
  function handleUnAssignToPublic(record: Recordable) {
    createConfirm({
      iconType: 'info',
      title: t('tb.dashboard.action.setSelfConfirm', { name: record.title }),
  ...
  function isSelectedMapTemplate(record: Recordable) {
    return Boolean(record?.id?.id && selectedMapTemplateId.value === record.id.id);
  }

  function handleSetMapHomeTemplate(record: Recordable) {
    const dashboardId = record?.id?.id;
    if (!dashboardId) return;

    selectedMapTemplateId.value = dashboardId;
    saveSelectedMapTemplateId(currentUserId, dashboardId);
    showMessage(`已设为大屏：${record.title || ''}`, 'success');
    reload();
  }
</script>
