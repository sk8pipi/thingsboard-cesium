...
## 设备配置驱动大屏展示（2026-09-12）

用户批准按 ThingsBoard 设备配置 ID 统一地图图标和详情类型。模板可按配置指定数据/视频详情入口；默认仅精确 camera 配置使用视频入口，自定义配置可在点位样式面板指定。展示规则不会创建视频绑定或授予权限，视频身份和能力继续遵守本文的 tbDeviceId / Video API 边界。元数据、图片、迁移和回退以 docs/ai/map-device-profile-architecture.md 为准。
